...
export function CriarQuestao({ conteudos, selectedLinguagem, onQuestaoCriada, tipo }: CriarQuestaoProps) {
  ...
  const validateForm = () => {
    ...
    if (questaoForm.tipo === "quiz") {
      ...
      } else if (questaoForm.opcoes.some((opt: Alternativa) => !opt.texto.trim())) {
  ...
  const handleQuestaoSubmit = async () => {
    try {
      ...
      const dadosParaEnvio = questaoForm.tipo === "quiz" 
        ? {
            conteudo_id: questaoForm.conteudo_id,
            enunciado: questaoForm.enunciado.trim(),
            nivel: questaoForm.nivel,
            tipo: "quiz" as const,
            opcoes: questaoForm.opcoes?.map((op: Alternativa) => ({
              id: op.id,
              texto: op.texto.trim()
            })) || [],
            resposta_correta: questaoForm.opcoes?.find((op: Alternativa) => op.correta)?.id || "",
